...
import type { Assignment, Submission, Badge, Notification } from "../../types";
...
export default function StudentPortal({ studentId, displayName }: Props) {
  ...
  const due = assignments.filter((a) => a.status === "assigned").length;
  const submitted = assignments.filter((a) => a.status === "submitted").length;
  const graded = assignments.filter((a) => a.status === "verified").length;
...
// ── Assignment Card with file upload ─────────────────────────────────────────
